...
from SinGAN.customFuncs import *
...
if __name__ == '__main__':
    ...
    #    print("output already exist")
        ...
        for j in range(1, iter_num + 1, 1):
            real_ = imresize3D(real_, pow(1 / opt.scale_factor, 1), opt)
            ...
            m = nn.ZeroPad2d(5)
            #z_opt = m(z_opt)
            z_opt = nn.functional.pad(z_opt, (5, 5, 5, 5, 5, 5), 'constant', 0)
            Zs_sr.append(z_opt)
        out = SinGAN_generate(Gs_sr, Zs_sr, reals_sr, NoiseAmp_sr, opt, in_s=reals_sr[0], num_samples=1)
        out = out[:, :, 0:int(opt.sr_factor * reals[-1].shape[2]), 0:int(opt.sr_factor * reals[-1].shape[3]), 0:int(opt.sr_factor * reals[-1].shape[4])]
        dir2save = functions.generate_dir2save(opt)
        file_name = '%s/%d.pt' % (dir2save, 0)
        customFuncs.save3DFig(out, file_name)
